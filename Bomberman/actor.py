...
class Arena():
    ...
    def spawn(self, a: Actor, below: bool = False):
        """Register an actor into this arena.
        Actors are blitted in their order of registration.
        If `below` is True, the actor is added below all others.
        """
        if a not in self._actors:
            if below:
                self._actors.insert(0, a)  # Insert actor at the start of the list
            else:
                self._actors.append(a)  # Insert actor at the end (default)
